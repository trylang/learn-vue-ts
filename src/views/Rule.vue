<template>
  <div>
    ...
    <div class="editor">
      <editor v-if="DestroyIncomeStatistics" :initial-value="EditorContent" @onChange="onChangeHandler" id="tinyMce" :init="editorInit"></editor>
...
export default class EditorCon extends Vue {
  ...
  public DestroyIncomeStatistics: boolean = false;
  private rule: string = "";
  ...
  public getRule(type: string) {
    ...
    httpServer(param).then((res: any) => {
        // _this.$nextTick(() => {
        //   _this.EditorContent = res.data.rule;
        //   _this.initRule = res.data;
        //   _this.$emit("update:EditorContent", res.data.rule);
        // });
        _this.DestroyIncomeStatistics = true;
        _this.EditorContent = res.data ? res.data.rule : '';
        _this.initRule = res.data || {};
        _this.$emit('update:EditorContent', res.data ? res.data.rule : '');
        _this.$forceUpdate();
    });
  }
  public mounted() {
