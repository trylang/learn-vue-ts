<template>
  <div>
    ...
    <div class="content">
      ...
      <div class="right" >
        <header v-if="auditType=='view'">
          ...
          <p v-if="detail.state==2">驳回理由：<span>{{detail.reason}}</span></p>
        ...
        <div class="detail">
          <div class="item">
            <p>订单号：<span>{{detail.receiptNumber}}</span></p>
        ...
        <div class="content" v-if="auditType=='audit'">
          ...
          <el-form class="form" :model="detail" label-width="100px">
            <el-form-item label="订单号：">
              <el-input placeholder="请填写完整订单号" @change="ifverify=-1;" v-model="detail.receiptNumber"></el-input>
            </el-form-item>
            <el-form-item label="消费日期：">
              <el-date-picker type="date" placeholder="请选择消费日期" @change="ifverify=-1;" v-model="detail.shoppingTime" value-format="yyyy-MM-dd" style="width: 100%;"></el-date-picker>
            </el-form-item>

            <el-form-item class="store" label="消费商户：">
              <el-select v-model="detail.storeId" @change="ifverify=-1;" placeholder="请选择">
                <el-option
            ...
            <el-form-item v-if="detail.state == 1" style="width: 70%; height: 40px;" class="number-input" label="消费金额：">
              <el-input-number placeholder="请填写当前小票消费金额" v-model="detail.shoppingFee"></el-input-number>
              <span style=" position: relative; top: -2.5rem; left: 9rem; display: block; margin-left: 10px; width: 100px; color: #FF7633; cursor: pointer;" @click="search(detail.shoppingFee)">查询积分</span>
            </el-form-item>
            ...
            <el-form-item v-if="detail.state == 2" label="驳回理由：">
              <el-input placeholder="请填写驳回理由" type="textarea" :rows="2" :maxlength="30" v-model="detail.reason"></el-input>
    ...
    <transition name="fade">
      <section class="bgSection" v-if="showImg"  @click="viewImg">
        <el-carousel :autoplay="false" height="450px" indicator-position="outside" trigger="click">
            <el-carousel-item v-for="item in detail.receiptPhotos" :key="item">
              <img style="width: 96%; height: 100%; object-fit: contain;" :src="item" alt="">
            </el-carousel-item>
          </el-carousel>
      </section>
    </transition>
  ...
  public search(fee: number) {
    ...
    const param = {
      ...
      params: {
        ...
        portalType: this.userRouter.subjectType || this.userInfo.subjectType || 1,
